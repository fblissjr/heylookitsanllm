...
- **`GET /v1/admin/models/{id}/status` says busy or idle.** `requests_active`
  was declared but never filled, so it was null for every model (the TODO
  blamed gguf alone); a client fell back to timing a generation, which queued
  behind the one it measured. It is now the model's generations in flight on
  either engine, and the new `requests_waiting` is the generation gate's
  queue, which is process-wide. Spec §4 updated.
...
- **Every response echoes the client's `X-Request-ID`** (except an unhandled
  ...
  that set their own id (`/v1/messages`, the generate route) are unchanged.
  `docs/api_integration.md` and the OpenAPI narrative updated.
