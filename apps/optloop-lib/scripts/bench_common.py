...
config loading, per-cycle structured logging, and coderef git helpers
for multi-repo optimization of mlx-lm and mlx-vlm internals.
"""
...
OPTLOOP_ROOT = SCRIPT_DIR.parent                # apps/optloop-lib/
REPO_ROOT = OPTLOOP_ROOT.parent.parent          # heylookitsanllm root
REPOS_DIR = OPTLOOP_ROOT / "repos"              # fork clones
...
def sync_barrier():
    ...
    Note: mx.eval() is MLX's GPU synchronization primitive, not Python's
    built-in eval(). It forces lazy computation graphs to execute.
    """
...
def build_cycle_data(
    ...
    decision: str,
    decision_reason: str,
    coderef_changes: dict | None = None,
) -> dict:
    """Build the per-cycle JSON structure.

    coderef_changes: optional dict with fork repo commit info, e.g.
        {"mlx-lm": {"branch": "...", "commit": "...", "diff_stat": "..."},
         "mlx-vlm": {"branch": "...", "commit": "...", "diff_stat": "..."}}
    """
    ...
    if coderef_changes is not None:
        cycle["git"]["coderef_changes"] = coderef_changes
    return cycle


# ---------------------------------------------------------------------------
# Coderef git helpers (multi-repo optimization)
# ---------------------------------------------------------------------------

def get_coderef_head(repo_name: str) -> dict | None:
    """Get branch + commit hash for a fork repo under repos/.

    Returns {"branch": "...", "commit": "..."} or None if repo not found.
    """
...
def snapshot_coderef() -> dict:
    """Snapshot HEAD of mlx-lm and mlx-vlm before a cycle.

    Returns {"mlx-lm": {"branch": ..., "commit": ...}, "mlx-vlm": {...}}.
    """
    snapshot = {}
    for name in ("mlx-lm", "mlx-vlm"):
        head = get_coderef_head(name)
        if head:
            snapshot[name] = head
    return snapshot


def reset_coderef(snapshot: dict):
    """Reset fork repos to pre-cycle state on discard.

    snapshot: dict from snapshot_coderef(), e.g.
        {"mlx-lm": {"branch": "main", "commit": "abc1234"}, ...}
    """
    for name, info in snapshot.items():
        repo_dir = REPOS_DIR / name
        if not repo_dir.is_dir():
            print(f"WARNING: repo dir {repo_dir} not found, skipping reset", file=sys.stderr)
            continue
        ...
        try:
            subprocess.run(
                ["git", "-C", str(repo_dir), "reset", "--hard", commit],
                capture_output=True, text=True, timeout=10,
            )
        except Exception as exc:
            print(f"WARNING: failed to reset {name} to {commit}: {exc}", file=sys.stderr)


def get_coderef_diff(repo_name: str) -> str:
    """Get diff stat of last commit in a fork repo.

    Returns the --stat output string, or empty string on failure.
    """
    repo_dir = REPOS_DIR / repo_name
    if not repo_dir.is_dir():
        return ""
    try:
        result = subprocess.run(
            ["git", "-C", str(repo_dir), "diff", "HEAD~1", "--stat"],
            capture_output=True, text=True, timeout=10,
        )
        if result.returncode == 0:
