#!/usr/bin/env python3
"""Benchmark results analysis tool for library-level optimization.
...
Usage:
    uv run apps/optloop-lib/scripts/bench_analysis.py
    uv run apps/optloop-lib/scripts/bench_analysis.py --no-chart
"""
...
# ---------------------------------------------------------------------------
# TSV parsing (extended columns for fork commit hashes)
# ---------------------------------------------------------------------------
...
def load_results_tsv() -> list[dict]:
    ...
    Expected columns:
        mlx_lm_commit  mlx_vlm_commit  text_score  vlm_score  text_tps
        vlm_tps  text_ttft  vlm_ttft  memory_gb  status  description
    """
...
def print_summary(rows: list[dict]):
    ...
    if kept:
        ...
        header = f"{'mlx-lm':<10} {'mlx-vlm':<10} {'Text':>8} {'VLM':>8} {'TxtTPS':>8} {'VlmTPS':>8} {'Description'}"
        print(header)
        print("-" * len(header))
        for r in kept:
            print(f"{r.get('mlx_lm_commit', '?'):<10} "
                  f"{r.get('mlx_vlm_commit', '?'):<10} "
...
def generate_chart(rows: list[dict], cycles: list[dict]):
    ...
    fig.suptitle("Library Optimization Progress")
    fig.tight_layout()
...
def main():
    parser = argparse.ArgumentParser(description="Library benchmark results analysis")
