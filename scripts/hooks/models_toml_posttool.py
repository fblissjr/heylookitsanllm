#!/usr/bin/env python3
"""Claude Code PostToolUse hook: validate models.toml right after an edit.

Wired in `.claude/settings.json` (matcher Edit|Write|MultiEdit). Reads the
hook payload on stdin and ignores any file that is not a `models.toml`. It
...
def main() -> int:
    ...
    if Path(file_path).name != "models.toml":
        return 0
    path = Path(file_path)
    if not path.is_file():
        return 0
    try:
        ...
        print(f"models.toml no longer parses as TOML: {exc}", file=sys.stderr)
        return 2
    try:
        from heylook_llm.config import AppConfig
        cfg = AppConfig(**data)
    except ImportError as exc:
        # The venv is not synced. That is not a models.toml problem, so do not
        # block the edit over it.
        print(f"models.toml hook skipped: {exc}", file=sys.stderr)
        return 0
    except Exception as exc:  # pydantic ValidationError and friends
        print(
            "models.toml fails the validation the server runs at startup, so "
