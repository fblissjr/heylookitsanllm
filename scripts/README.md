...
| `hooks/models_toml_posttool.py` | **Claude Code PostToolUse hook**, wired in `.claude/settings.json`: after an edit to `models.toml` it runs the validation the server runs at startup, and exits 2 with the error so the model sees it at edit time. Ignores every other file. | runs from Claude Code; manual: pipe a hook payload into it |
