...
| `chain_probe.py` | **MLX prompt-cache restore check** against a running server: each hop of a multi-turn chain (extends, then one edit) runs fresh and restored at temperature 0, and the texts must match with the restored run reporting `reused`. Every extend hop must restore (an edit may re-prefill); exits 1 on a mismatch or a missed required restore, and prints which hops restored. Run it on each model class a cache, trim or position change touches; unit tests on fakes cannot see this class. | `uv run python scripts/chain_probe.py --server URL --model ID [--out DIR]` |
