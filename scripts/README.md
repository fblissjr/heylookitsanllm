...
| `check_version_sync.py` | **Pre-commit guard** (wired into the git hook): the newest `## [x.y.z]` heading in `CHANGELOG.md` must equal `__version__`, read from the STAGED blobs. Stdlib only and fails closed. Replaced a hookify reminder that never ran while the plugin was disabled. | runs from the hook; manual: `scripts/check_version_sync.py` |
| `hooks/git_add_guard.py` | **Claude Code PreToolUse hook** (Bash), wired in `.claude/settings.json`: refuses `git add -A/--all/-u/--update/./:/` and `git commit -a/--all`, because parallel sessions share this checkout. Stage files by name. Heredoc bodies are ignored. | runs from Claude Code; `tests/unit/test_git_add_guard.py` |
| `hooks/config_toml_posttool.py` | **Claude Code PostToolUse hook**, wired in `.claude/settings.json`: after an edit to `heylook.toml` it runs the validation the server runs at startup, and exits 2 with the error so the model sees it at edit time. Ignores every other file. | runs from Claude Code; manual: pipe a hook payload into it |
...
| `gguf_probe.py` | Direct llama-server diagnostics for one GGUF model, BELOW `dev_server.sh` (no FastAPI/DB): /props modalities, thinking-template on/off diff, one-shot gen w/ tps + draft acceptance, LoRA attach + off/on A/B, auto-teardown. Sidecar pairing via the importer's pickers. | `uv run python scripts/gguf_probe.py <model-dir> [--spec-type draft-mtp] [--lora A.gguf --lora-ab]` |
...
| `chain_probe.py` | **MLX prompt-cache restore check**: each hop of a multi-turn chain (extends, then one edit) runs fresh and restored at temperature 0, and the texts must match with the restored run reporting `reused`. Every extend hop must restore (an edit may re-prefill); exits 1 on a mismatch or a missed required restore. In-process (the default) a divergence is judged at its first differing token with one fresh forward pass: a NEAR-TIE (the two picked tokens are the top two, within one bf16 quantum) is not a failure. `--server` probes a running server instead, with no tie verdicts. Run it on each model class a cache, trim or position change touches; unit tests on fakes cannot see this class. | `uv run python scripts/chain_probe.py --model ID [--server URL] [--out DIR]` |
| `perf_ab.py` | **Speed and memory A/B** of one model across code versions (git revs through worktrees, import verified) or configs (JSON merged into the model's config). One arm per fresh process, alternating rounds, warmup dropped, page cache primed, and a wait for a quiet machine before each arm. Samples process footprint (llama-server child included), MLX active/cache/peak, wired/swap/pressure, CPU and GPU (ioreg) during the run. `report` calls a change only when both arms have enough values, the ranges separate and the difference clears `MIN_EFFECT`; it withholds the overall verdict when other inference ran. Records go to `internal/claude/perf/` with their conditions. | `uv run python scripts/perf_ab.py run --model ID --arm base=HEAD~1 --arm new=.` · `... report FILE` |
| `_inproc.py` | Shared by the in-process instruments: the router's config resolution (validated) and running MLX work on one pinned thread in mlx-vlm's generation stream; `NEAR_TIE_MARGIN`. | imported |
...
| Setting | Why |
| --- | --- |
| `GGML_METAL_EMBED_LIBRARY=ON` | the Metal shader SOURCE inside the binary, compiled by the OS at startup (`newLibraryWithSource`) -- it moves without dragging a shader file along, and shader code generation tracks the installed macOS, not Xcode (which is also why Xcode's optional Metal Toolchain component is not needed). Upstream's own macOS release build sets exactly this. |
