...
try:
    ...
    from .metrics_db import get_metrics_db as _get_metrics_db
    from .metrics_db import init_metrics_db as _init_metrics_db
except ImportError:
    DUCKDB_AVAILABLE = False
    logger.info("DuckDB not installed. Analytics features will be disabled.")
    logger.info("Install with: uv sync --extra all   ")
    ...
    class MetricsDB:
        ...
        def log_model_switch(
            self, from_model: str, to_model: str, unload_time: float, load_time: float
        ):
            pass

        def execute_query(self, query: str, limit: int = 1000) -> Dict[str, Any]:
            return {
                "error": "Analytics not available. Run: uv sync --extra analytics"
            }
