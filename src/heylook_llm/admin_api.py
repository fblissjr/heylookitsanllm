...
# NOTE: these MUST stay above the bare `/{model_id:path}` routes below.
# That converter is greedy -- it matches slashes -- so a route declared
# after it never sees `<id>/chat-template`: the catch-all claims the whole
# tail as the model id and the handler 404s on a model that does exist.
# Every other sub-path route here (/status, /fit, /reload, ...) is above it
# for the same reason.
@admin_router.get(
    "/{model_id:path}",
