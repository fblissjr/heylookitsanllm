...
class ModelTemplateInfo:
    ...
    # Template reads ``reasoning_effort``. DISTINCT from
    # supports_enable_thinking, and deliberately so: gpt-oss/harmony reads
    # reasoning_effort and has no enable_thinking at all, so deriving one from
    # the other hides the depth control on the family that most needs it (it
    # is always-reasoning, making depth its ONLY thinking knob).
...
def read_template_info(
    ...
    supports_reasoning_effort = bool(_REASONING_EFFORT_PATTERN.search(template))
