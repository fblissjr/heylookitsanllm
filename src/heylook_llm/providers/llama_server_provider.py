...
class LlamaServerProvider(BaseProvider):
    ...
    # Where scripts/update_deps.py builds llama-server when nothing overrides
    # it. Kept in sync with that script's own default by a test.
    ...
    def _resolve_binary(self) -> Path:
        ...
        The last fallback exists because the other two are both fragile in ways
        that bite silently. `models.toml` is REGENERATED by `heylookllm import`,
        so a server_binary written there disappears on the next import; and an
        exported env var goes stale the moment a newer binary is built,
        pointing a working server at an old build with nothing to say so.

        Falling back to our own build location is safe precisely because
        update_deps.py owns that path -- if the file is not there, nothing has
        ...
        if not candidate and self.DEFAULT_BUILD.is_file():
            logging.info(
                f"[GGUF] no server_binary or $HEYLOOK_LLAMA_SERVER set; using the "
                f"build from scripts/update_deps.py at {self.DEFAULT_BUILD}"
            )
            return self.DEFAULT_BUILD
        if not candidate:
            raise RuntimeError(
                "No llama-server binary configured and none built. Run "
                "`uv run scripts/update_deps.py llama.cpp`, or set server_binary "
                "in models.toml / the $HEYLOOK_LLAMA_SERVER env var."
            )
