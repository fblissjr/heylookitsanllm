...
`$HEYLOOK_LLAMA_SERVER`, else it FALLS BACK to the binary `update_deps.py` built (fixed
dir under home); only if none of the three exists does load fail loudly. The fallback
exists because the other two rot silently: models.toml is REGENERATED by `heylookllm
import` so a `server_binary` written there vanishes, and an exported env var keeps
pointing at an old build after you build a new one, BUILT by `scripts/update_deps.py llama.cpp` (the only thing that clones/builds
...
- `models.toml` COMMENTS ARE EPHEMERAL -- do not record reasoning there. `model_service._write_toml` is a full-file rewrite through `tomli_w`, which emits no comments, so ANY admin write (import, PATCH, config edit) silently drops every one. Verified 2026-08-10: 26 comment lines -> 0 on a single round-trip. The file looks hand-maintained and the loss happens on an unrelated action, which is what makes it bite. Consequence worth knowing: models.toml cannot carry provenance for its own settings -- where a value came from belongs in CLAUDE.md or `internal/`, not next to the value. A FIX IS WANTED, not a settled state (owner approved 2026-08-10, with the constraint that it must not become a source of drift). Design that was tried and reverted: keep `tomli_w` authoritative for VALUES, use tomlkit to carry commentary forward, and preserve a model's comments ONLY while that model's values are byte-identical -- so a note can never outlive what it describes. It failed `test_import_reimport.py` because splicing whole tables from the old document carries their trivia and serialises to malformed TOML; `_write_toml`'s validation caught it and refused the write. The likely fix is to copy only the COMMENTS onto a freshly rendered table.
