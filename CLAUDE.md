...
- Root venv: plain `uv sync` is the whole story now (v1.39.17). The performance stack (pyturbojpeg, uvloop, xxhash, cachetools) + CLI (questionary) are CORE deps, and dev tooling (pytest+plugins, httpx, build, twine, rich, py-spy) is the `dev` dependency-group uv installs by default -- there are NO optional extras anymore, and no `--all-extras` to forget. `uv sync --no-dev` for a runtime-only install. Bump the git-pinned mlx-lm/mlx-vlm revs with `scripts/update_deps.py` (index: `scripts/README.md`).
