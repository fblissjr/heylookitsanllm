...
- Spec decode is ON by default whenever a model ships a drafter (owner decision 2026-09-24, replacing "per-model opt-in"): a drafter file, or an MTP head built into the GGUF. The off switch is per model: today an explicit models.toml entry without `draft_model_path` or `spec_type`, later the model's own sidecar file. A model served with a LoRA (steering included) should have it off, because the drafter never gets the adapter and spec decode is fixed when the process spawns.
- What discovery does today falls short of that decision. It pairs only a drafter file beside the weights (named `mtp-`/`dspark-`/`dflash-`/`eagle3-`). It does not yet detect a built-in MTP head (`blk.N.nextn.*` in the model's header; Qwen3.8-27B, Qwen3.6), a drafter in a subfolder (`MTP/`), or a drafter for the same model in a neighbouring folder (DeepSeek V4 Flash Vision's quants). Header-based detection is planned with the registry sidecars (`docs/project/plan_registry_sidecars.md`). An explicit models.toml entry receives none of it either way. Until then, turn it on per model: `spec_type = "draft-mtp"` alone for a built-in head, `draft_model_path` for a drafter file.
...
- Discovery auto-pairs `draft_model_path` for a sidecar named `mtp-`/`dspark-`/`dflash-`/`eagle3-` and leaves `spec_type` unset, so such a model runs spec decode at llama.cpp's defaults with no models.toml entry. To find those models, walk `merge_discovered(data, discover(data))` for configs carrying `draft_model_path`.
