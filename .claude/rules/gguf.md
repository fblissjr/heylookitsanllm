...
- Spec decode is ON by default whenever a model ships a drafter (owner decision 2026-09-24, replacing "per-model opt-in"): a drafter file, or an MTP head built into the GGUF. The off switch is per model: `unset = ["draft_model_path", "spec_type"]` in the model's `model.heylook.toml`, or an explicit models.toml entry without those fields. A model served with a LoRA (steering included) should have it off, because the drafter never gets the adapter and spec decode is fixed when the process spawns.
