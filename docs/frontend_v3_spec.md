...
degrades into paging). `sysctl_suggest_mb` is non-null ONLY while `iogpu.wired_limit_mb` is at its
OS default AND the working set is exceeded or `headroom_thin` is true — show
the sysctl hint iff present. `headroom_thin` (v2.0.13) = working-set headroom
...
The body (plan W1, v2.0.136; it replaced v1.79.61's `?ctx_size=N` query) is an
object of the provider's `load_setting` fields from `/v1/admin/model-options`
...
`POST /{id}/unload` →
`{status:"unloaded"|"not_loaded"}`; **409** while the model is generating. `POST /scan` and `POST /import` were retired in v2.0.72 with
...
enum?,arg?,ui?,shape?,reason?,load_setting?}]}}}` — every settable heylook.toml key per provider, derived
...
test); `load_setting:true` (v2.0.136) marks a field the load panel offers and `/reload`'s
body takes; `ui:"advanced"` collapses the field into an Advanced group; `ui:"hidden"` means
...
family. v3 chat calls it at staging, after a model switch and after each
residency refresh, for each staged image's size and its original's size: the
