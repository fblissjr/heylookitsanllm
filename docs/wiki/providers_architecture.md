...
- `cache` -- how the model's prompt is reused across requests, as Facts. gguf gives the reuse class read from the header (a recurrent or sliding-window model is checkpointed, full attention truncates anywhere), KV shift (off whenever a projector loads), the host-RAM prompt-cache budget and the checkpoint settings, each as the spawn will get it. MLX gives the prefix cache (mlx-vlm's APC, in memory per loaded model): its reuse mode (checkpoints or hashed blocks) and whether images reuse, both decided at load; the checkpoint interval, the checkpoints one request captures and the store size (`vlm_engine` constants); the memory budget; and that the disk tier is off. What one request actually reused is its `usage`, not this.
- `thinking` -- the thinking controls the in-force template offers, found by rendering it ([`thinking_controls.detect`](../../src/heylook_llm/thinking_controls.py)): the switch, and the depth variable with its values in the template's own spellings, aliases, default, how it treats an unknown value, and whether depth enters the prompt before the first user turn, plus `template`, the copy it was read from, and `budget` (`{enforced, reason}`): whether a thinking budget can be enforced, known on MLX (heylook forces the close) and unknown on gguf (llama-server's own parser decides per request). The copy is the one the engine's own template ladder picks, so a `chat_template.jinja` beside the weights decides the levels over a GGUF's embedded template, and on MLX over `tokenizer_config.json`; a template with a switch and no depth offers on/off only. `depth.off` lists the values that render the same prompt as the switch set to false, so v3's single thinking control folds them into its Off. Within a group of words that render the same prompt, the word the template itself normalizes to names the level (`set effort = 'low'`), else the first one the template uses after the variable's first mention, so a word used earlier for something else never names a depth. It is the only list of depth values anywhere; the capability, the request check and the UI all read it.
- `decoding` -- `mode` (autoregressive, or diffusion for a masked-diffusion checkpoint) and `request_fields`, the request fields that engine path reads (null = all). gguf answers autoregressive from the start; MLX decides at load with mlx-vlm's own check on the loaded model, and a diffusion model's list is `mlx_provider.DIFFUSION_REQUEST_FIELDS`, kept beside the strategy that reads them. The sampler panel hides a control whose field is not listed.
- `speculative` -- `drafter` (the file, the built-in MTP head, or none, and where discovery found it or that the model's own file turned it off), `type` (the draft type llama.cpp runs: pinned, or inferred from the drafter's header), and `in_force` (whether the running process drafts; when it does not, why: the drafter did not fit in live memory at spawn, llama-server could not load it, or none is set). MLX answers not applicable: heylook's MLX path does not draft.
- `image`, `steering` -- explicit nulls until the workstreams that fill them land.
...
The Messages wire's `response_format` is normalized to `ChatRequest.response_schema` (None = free text, `{}` = any object). gguf hands it to llama-server as its own `json_schema` field; llama-server's chat parser admits the reasoning block and then holds the reply to the schema, returning only the JSON as content. MLX adds mlx-vlm's llguidance processor (`build_json_schema_logits_processor`) to the request's logits processors, wrapped in `ThinkingAwareLogitsProcessor` when thinking is on so the constraint starts at the same closer the thinking budget uses (`thinking_budget_markers`). Where that start cannot be found (harmony, masked diffusion, a continuation) the provider refuses rather than constrain the wrong tokens or none.
