...
- **Per-request reporting (v2.0.78).** `performance.speculative` carries drafted (gguf only), accepted and emitted, with `acceptance_rate` (accepted/drafted) and `draft_share` (accepted/emitted) as separate fields: the engines count different things, and one ratio name used to carry either. `performance.cache` says what each request reused of its prompt. The per-message stats line and the perf page's cache section follow in the [runtime visibility plan](../project/plan_runtime_visibility.md)'s W5.
