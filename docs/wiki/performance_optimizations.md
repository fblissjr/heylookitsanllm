...
- [`vision_feature_cache.py`](../../src/heylook_llm/providers/common/vision_feature_cache.py) maintains an LRU cache of encoded vision features. [The vision strategy](../../src/heylook_llm/providers/mlx_provider.py) uses it two ways. A model with `encode_image()` (gemma4, lfm2_vl, molmo and others) has its tower run by heylook, and the features reach the model as `cached_image_features`. A model without it (qwen3_5) is handed the cache as mlx-vlm's `vision_cache` plus `_image_key` kwargs, the way mlx-vlm's own server does, and looks up and stores its own tower output.
