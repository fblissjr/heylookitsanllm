...
**One hard constraint the sizing does not decide, so a second rule guards it.** Some projectors decode an image's tokens with non-causal attention, and llama.cpp asserts that such a batch fits in one micro-batch -- so an image with more tokens than the micro-batch would abort the process. `_image_token_cap()` reads the projector type from the mmproj header at spawn. For a projector in `NON_CAUSAL_IMAGE_PROJECTORS` whose default per-image maximum exceeds the effective micro-batch (the stored or auto `-ub`, else llama-server's default, clamped to `-b` as llama.cpp does), the spawn passes `--image-max-tokens` equal to that micro-batch, and logs the decision either way. It never raises a projector's default, it defers to an `--image-max-tokens` in `extra_args`, and for a fixed-count projector the flag cannot lower (gemma3) it warns instead. Causal projectors are never capped, since that would only cost image resolution. The table is a hand copy of llama.cpp source, pinned by a test against the built tree; why it is a table and not derived: [sharp_edges.md#gguf-non-causal-images](../architecture/sharp_edges.md#gguf-non-causal-images).
