...
- **What is reported.** `llama-server` reports only the final cached count, not whether it came from the slot, the RAM cache or a checkpoint; where a restore came from is trace-level and never printed at the default verbosity. heylook reports that count per request (`usage.cache_read_input_tokens`, `performance.cache`) and adds a `cause` when a request reused less than it could have, from the **cache witness** ([`llama_cache_witness.py`](../../src/heylook_llm/providers/llama_cache_witness.py)), one per process:
  ...
  - A first request, or the first after a sleep, is `cold`; a request sharing no first message with any remembered one is `no_common_prefix`. Only a request that fully extends a remembered one says how much should have been reusable (that request's whole prompt); reusing well under it is `probable_template_diverged` when the slot held it, and otherwise `probable_budget_skipped` or `probable_evicted` when the log shows one since. `probable_` means inferred, not reported. The durable fix is upstream: a cache source in `timings`.
