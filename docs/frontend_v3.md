...
js/
  ...
  engine.js                 # the engine contract's one reader and renderer: readFact, contextCeiling/contextRunning, renderEngine (models-page row panel: facts, later slots generically, settings grouped by effect, ui:"hidden" fields only when configured), renderEngineCompact (chat's engine chip panel)
  load-panel.js             # chat's load panel (createLoadPanel, plan W1): one control per `load_setting` field from /v1/admin/model-options beside the model select -- the context select below, and an Auto-first select per other field labelled with what auto resolved to; choicesToSend() is the /reload body (null = Auto)
  context-select.js         # the load panel's context-size select for the NEXT gguf load (createContextSelect): power-of-two steps up to the header's ceiling, Auto = llama-server's own sizing, rebuilt only when the model's facts move so a fresh pick survives a residency refresh
